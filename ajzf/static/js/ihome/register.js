...
$(document).ready(function() {
    ...
    $("#btn-reg").on('click', function(e){
        ...
        passwd2 = $("#password2").val().trim();
        if (!mobile) {
            $("#mobile-err span").html("手机号不能为空！");
            $("#mobile-err").show();
            return;
        } 
        if (!username) {
            $("#username-err span").html("用户名不能为空！");
            $("#username-err").show();
            return;
        }
        if (!passwd) {
        ...
        $.ajax({
            url: '/user/register/',
            type: 'post',
            data: {'phone': mobile, 'username': username, 'password': passwd, 'password2': passwd2},
            dataType: 'json',
